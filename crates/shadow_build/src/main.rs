...
pub struct Args {
    #[clap(long)]
    version: String,
    #[clap(long, default_value = "https://github.com/TheShadowEngine/shadowrs")]
    pkgs_url: String,
}
...
fn compile() {
    ...
    for target in TARGETS {
        ...
        // cli
        let shadowrs_cli_src = cargo_artifact_path.join(target_file_names.shadowrs_cli_file_name);
        let shadowrs_cli_dst = dist_target_path.join(target_file_names.shadowrs_cli_file_name);
        std::fs::copy(shadowrs_cli_src, shadowrs_cli_dst).unwrap();
        // shadowrs.h
        let shadowrs_h_dst = dist_target_path.join(target_file_names.shadowrs_h_file_name);
        cbindgen::generate(root_path.join("languages/c"))
            .unwrap()
            .write(std::fs::File::create(shadowrs_h_dst).unwrap());
        // libshadowrs dynamic
        std::fs::copy(
            cargo_artifact_path.join(target_file_names.libshadowrs_dynamic_file_name),
            dist_target_path.join(target_file_names.libshadowrs_dynamic_file_name),
        )
        .unwrap();
        // libshadowrs static
        std::fs::copy(
            cargo_artifact_path.join(target_file_names.libshadowrs_static_file_name),
            dist_target_path.join(target_file_names.libshadowrs_static_file_name),
        )
        .unwrap();
        // shadowrs_elixir
        std::fs::copy(
            cargo_artifact_path.join(target_file_names.shadowrs_elixir_file_name),
            dist_target_path.join(target_file_names.shadowrs_elixir_file_name),
        )
        .unwrap();
        // shadowrs_node
        std::fs::copy(
            cargo_artifact_path.join(target_file_names.shadowrs_node_file_name),
            dist_target_path.join(target_file_names.shadowrs_node_file_name),
        )
        .unwrap();
        // shadowrs_python
        std::fs::copy(
            cargo_artifact_path.join(target_file_names.shadowrs_python_file_name),
            dist_target_path.join(target_file_names.shadowrs_python_file_name),
        )
        .unwrap();
    }
    // shadowrs_wasm
    ...
    std::fs::create_dir(&dist_target_path).unwrap();
    std::fs::copy(
        cargo_artifact_path.join("shadowrs_wasm.wasm"),
        dist_target_path.join("shadowrs_wasm.wasm"),
    )
    .unwrap();
}
...
fn build_debs(version: &str) {
    ...
    for target in [Target::AArch64LinuxGnu, Target::X8664LinuxGnu] {
        ...
        // Copy the shadowrs cli to the deb's /usr/bin.
        let shadowrs_cli_file_name = TargetFileNames::for_target(target).shadowrs_cli_file_name;
        let shadowrs_cli_path = root_path
            .join("dist")
            .join("compile")
            .join(target.target_name())
            .join(shadowrs_cli_file_name);
        std::fs::copy(shadowrs_cli_path, bin_path.join(shadowrs_cli_file_name)).unwrap();
        ...
        let deb_file_name = format!(
            "shadowrs_{version}_{architecture}.deb",
            version = version,
            architecture = architecture,
        );
...
fn build_rpms(version: &str) {
    ...
    for target in [Target::AArch64LinuxGnu, Target::X8664LinuxGnu] {
        ...
        // Make the tar.
        let shadowrs_cli_file_name = TargetFileNames::for_target(target).shadowrs_cli_file_name;
        let shadowrs_cli_path = compile_path
            .join(target.target_name())
            .join(shadowrs_cli_file_name);
        let shadowrs_path_in_tar =
            PathBuf::from(format!("shadowrs-{version}/shadowrs", version = version));
        let sources_path = rpm_path.join("SOURCES");
        let tar_path = sources_path.join("shadowrs.tar.gz");
        tar(&[(shadowrs_cli_path, shadowrs_path_in_tar)], &tar_path);
        // Write the spec file.
        let spec = formatdoc!(
            r#"
				%global __strip true
				...
				install -m 755 shadowrs %buildroot/usr/bin/shadowrs

				%files
				%attr(0755, root, root) /usr/bin/shadowrs
			"#,
            version = version,
        );
        let spec_path = rpm_path.join("SPECS/shadowrs.spec");
        ...
        let src_rpm_file_name = format!(
            "shadowrs-{version}-1.{target}.rpm",
            version = version,
            target = target,
        );
        let dst_rpm_file_name = format!(
            "shadowrs_{version}_{target}.rpm",
            version = version,
            target = target,
        );
        std::fs::copy(
...
fn build_containers(version: &str) {
    ...
    for target in [Target::AArch64LinuxMusl, Target::X8664LinuxMusl] {
        ...
        let shadowrs_cli_path = compile_path
            ...
            .join(target.target_name())
            .join(shadowrs_cli_file_name);
        let shadowrs_cli_path = shadowrs_cli_path.display();
        let dockerfile = formatdoc!(
            r#"
				FROM docker.io/alpine
				WORKDIR /
				COPY {shadowrs_cli_path} .
				ENTRYPOINT ["/shadowrs"]
			"#,
            shadowrs_cli_path = shadowrs_cli_path,
        );
        ...
        let tag = format!(
            "docker.io/shadowrsdotdev/shadowrs:{version}",
            version = version,
        );
        cmd!(
            "podman",
...
fn alpine(
    ...
    for target in [Target::AArch64LinuxMusl, Target::X8664LinuxMusl] {
        ...
        std::fs::copy(alpine_public_key_path, src_path.join("shadowrs.rsa")).unwrap();
        ...
        let apkbuild = formatdoc!(
            r#"
				...
				pkgver={version}
				pkgrel=1
				pkgdesc="Machine Learning framework"
				arch={arch}
				license="MIT"
				source="shadowrs"
				options="!strip"
				...
				package() {{
					install -D -m 755 "$srcdir"/shadowrs "$pkgdir"/usr/bin/shadowrs
				}}
			"#,
            version = version,
            arch = arch,
        ...
        let shadowrs_cli_file_name = TargetFileNames::for_target(target).shadowrs_cli_file_name;
        let shadowrs_cli_dst_path = src_path.join("shadowrs");
        let shadowrs_cli_path = compile_path
            .join(target.target_name())
            .join(shadowrs_cli_file_name);
        std::fs::copy(shadowrs_cli_path, &shadowrs_cli_dst_path).unwrap();
    ...
    for distribution in distributions {
        ...
        for distribution_version in distribution_versions {
            ...
            let list_file = formatdoc!(
                r#"
					# shadowrs packages for {distribution} {distribution_version}
                ...
                for deb in debs.iter().filter(|deb| deb.arch == arch) {
                    ...
                    let packages_entry = formatdoc!(
                        r#"
							...
							Version: {deb_version}
							Architecture: {arch}
							Filename: pool/shadowrs_{version}_{arch}.deb
							Size: {size}
							MD5sum: {md5}
							SHA1: {sha1}
							SHA256: {sha256}
...
fn rpm(pkgs_url: &str, version: &str, rpm_public_key_path: &Path, rpm_private_key_path: &Path) {
    ...
    for (distribution, distribution_version) in [
        ...
        let repo_file_path = repo_path.join("shadowrs.repo");
...
fn build_release(version: &str) {
    ...
    for target in TARGETS {
        ...
        let output_path = release_path.join(format!(
            "shadowrs_cli_{version}_{target_name}.tar.gz",
            version = version,
            target_name = target_name,
        ));
        let inputs = vec![(
            shadowrs_cli_path.clone(),
            PathBuf::from(shadowrs_cli_file_name),
        )];
    ...
    for target in TARGETS {
        ...
        let target_name = target.target_name();
        let output_path = release_path.join(format!(
            "libshadowrs_{version}_{target_name}.tar.gz",
            version = version,
            target_name = target_name,
        ));
        let inputs = vec![
            (
                target_path.join(target_file_names.shadowrs_h_file_name),
                PathBuf::from(target_file_names.shadowrs_h_file_name),
            ),
            (
                target_path.join(target_file_names.libshadowrs_dynamic_file_name),
                PathBuf::from(target_file_names.libshadowrs_dynamic_file_name),
            ),
            (
                target_path.join(target_file_names.libshadowrs_static_file_name),
                PathBuf::from(target_file_names.libshadowrs_static_file_name),
            ),
        ];
        tar(&inputs, &output_path);
    }
...
pub struct TargetFileNames {
    pub shadowrs_cli_file_name: &'static str,
    pub shadowrs_h_file_name: &'static str,
    pub libshadowrs_dynamic_file_name: &'static str,
    pub libshadowrs_static_file_name: &'static str,
    pub shadowrs_elixir_file_name: &'static str,
    pub shadowrs_node_file_name: &'static str,
    pub shadowrs_python_file_name: &'static str,
}
    ...
    pub fn for_target(target: Target) -> TargetFileNames {
        match target {
            ...
            | Target::AArch64LinuxMusl => TargetFileNames {
                shadowrs_cli_file_name: "shadowrs",
                shadowrs_h_file_name: "shadowrs.h",
                libshadowrs_dynamic_file_name: "libshadowrs.so",
                libshadowrs_static_file_name: "libshadowrs.a",
                shadowrs_elixir_file_name: "libshadowrs_elixir.so",
                shadowrs_node_file_name: "libshadowrs_node.so",
                shadowrs_python_file_name: "libshadowrs_python.so",
            },
            Target::X8664MacOs | Target::AArch64MacOs => TargetFileNames {
                shadowrs_cli_file_name: "shadowrs",
                shadowrs_h_file_name: "shadowrs.h",
                libshadowrs_dynamic_file_name: "libshadowrs.dylib",
                libshadowrs_static_file_name: "libshadowrs.a",
                shadowrs_elixir_file_name: "libshadowrs_elixir.dylib",
                shadowrs_node_file_name: "libshadowrs_node.dylib",
                shadowrs_python_file_name: "libshadowrs_python.dylib",
            },
            Target::X8664WindowsMsvc => TargetFileNames {
                shadowrs_cli_file_name: "shadowrs.exe",
                shadowrs_h_file_name: "shadowrs.h",
                libshadowrs_dynamic_file_name: "shadowrs.dll",
                libshadowrs_static_file_name: "shadowrs.lib",
                shadowrs_elixir_file_name: "shadowrs_elixir.dll",
                shadowrs_node_file_name: "shadowrs_node.dll",
                shadowrs_python_file_name: "shadowrs_python.dll",
            },
